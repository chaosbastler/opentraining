...
/**
 * This class should be used for "tagging" an ExerciseType. It should behave
 * like an enum, though it isn't one.
 * 
 * Reason: Later the ExerciseTags should be dynamically read at runtime. Why?
 * Two main reasons: - different languages(English, German, ...) - easy adding
 * new ExerciseTags, perhaps even for users
 * 
 * Note: Enum may be a misleading comparison as TypeSavety can't be achieved by
 * dynamically adding the values. But as this class once was an enum
 * class(before refactoring it), the term enum seems to be appropriate.
 * 
 * Behavior that is very similar to an enumeration: - only one instance per name
 * - the class cares about initializing the instances, neither user nor
 * programmer do have to care about that - Comparable<ExerciseTag> - method
 * values()
 * 
 ...
 */
public class ExerciseTag implements Comparable<ExerciseTag>, Serializable {
